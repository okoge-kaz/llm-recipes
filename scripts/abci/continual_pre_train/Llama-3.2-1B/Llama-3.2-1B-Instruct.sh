...
TOKENIZER_MODEL=/bb/llm/gaf51275/hf-checkpoints/Llama-3.2-1B/tokenizer.json
CHECKPOINT_DIR=/bb/llm/gaf51275/hf-checkpoints/Llama-3.2-1B-Instruct
CHECKPOINT_SAVE_DIR="/bb/llm/gaf51275/checkpoints/llm-recipes/Llama-3.2-1B-Instruct/LR_${LR}_minLR_${MIN_LR}_WD_${WEIGHT_DECAY}_GC_${GRAD_CLIP}"
...
# job name
JOB_NAME="Llama-3.2-1B-Instruct-ABCI--${NODE_TYPE}-${NUM_NODES}node-${NUM_GPUS}gpu-${SEQ_LENGTH}s-BS=${GLOBAL_BATCH_SIZE}-LR=${LR}-MINLR=${MIN_LR}-WARMUP=${LR_WARMUP_STEPS}-WD=${WEIGHT_DECAY}-GC=${GRAD_CLIP}"
