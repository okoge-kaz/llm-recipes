...
# Control variables
USE_ENGLISH_LMSYS=false
USE_SYNTH_EN_WO_TEMPLATE=false
USE_SYNTH_JA_WO_TEMPLATE=false
USE_MAGPIE_ULTRA_EN=false
...
if $USE_SYNTH_JA_WO_TEMPLATE; then
  JA_LMSYS_FILE=/bb/llm/gaf51275/datasets/raw/instruct/lmsys-chat-1m/sft/lmsys-chat-1m-synth-ja-wo-pii-and-template-instructions-train.jsonl
else
...
if $USE_ENGLISH_LMSYS; then
  if $USE_SYNTH_EN_WO_TEMPLATE; then
    EN_LMSYS_FILE=/bb/llm/gaf51275/datasets/raw/instruct/lmsys-chat-1m/sft/lmsys-chat-1m-synth-en-wo-pii-and-template-instructions-train.jsonl
  else
...
if $USE_MAGPIE_ULTRA_EN; then
  MAGPIE_ULTRA_FILE=/bb/llm/gaf51275/datasets/raw/instruct/synthetic/magpie-ultra-v0.1/data/train_en.jsonl
  cat $MAGPIE_ULTRA_FILE >> $OUTPUT_DIR/train.jsonl
  echo "Added magpie-ultra en data"
fi

if $USE_MAGPIE_ULTRA_JA; then
  MAGPIE_ULTRA_FILE=/bb/llm/gaf51275/datasets/raw/instruct/synthetic/magpie-ultra-v0.1/data/train_ja.jsonl
  cat $MAGPIE_ULTRA_FILE >> $OUTPUT_DIR/train.jsonl
