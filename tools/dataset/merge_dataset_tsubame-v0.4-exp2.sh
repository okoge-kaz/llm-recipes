...
cat $MAGPIE_ULTRA_FILE >> $OUTPUT_DIR//train_mixed.jsonl
...
cat $GEMMA_MAGPIE_FILE >> $OUTPUT_DIR//train_mixed.jsonl
...
wc -l $LMSYS_FILE
cat $LMSYS_FILE >> $OUTPUT_DIR//train_mixed.jsonl
...
wc -l $EN_LMSYS_FILE
cat $EN_LMSYS_FILE >> $OUTPUT_DIR//train_mixed.jsonl
...
cat $MAGPIE_ULTRA_EN_FILE >> $OUTPUT_DIR//train_mixed.jsonl
echo "Added magpie-ultra en data"

CODE_FILE=/gs/bs/tga-NII-LLM/datasets/raw/instruct/training/v0.4/code_sample_1m.jsonl
echo "code file: $CODE_FILE"
wc -l $CODE_FILE
cat $CODE_FILE >> $OUTPUT_DIR//train_mixed.jsonl
echo "Added code data"

echo "Total data:"
wc -l $OUTPUT_DIR//train_mixed.jsonl

# indexing

python tools/pre-process/index_dataset.py \
  --data-file-path $OUTPUT_DIR//train_mixed.jsonl
